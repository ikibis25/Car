...
$sub=$_POST['usersub'];
$u_msg=$_POST['usermsg'];
if(trim($name)!="Your Name" && trim($email)!="Your Email" && trim($phone)!="Your Phone Number" && trim($sub)!="Your Subject" && trim($u_msg)!="Your Message" && trim($name)!="" && trim($email)!="" && trim($phone)!="" && trim($sub)!="" && trim($u_msg)!="")
{
	if(filter_var($email, FILTER_VALIDATE_EMAIL))
	{
		...
		if(mail('support@templatebundle.net','Query for Driving School',$message,$headers ))
		{
		mail($email,'Reply from Driving School Template Team',$message_user,$headers );
